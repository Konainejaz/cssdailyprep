...
const StudyEssaysView: React.FC<Props> = ({
  items,
  isLoading,
  onBack,
  onSaveNote,
          ...
          {isLoading ? (
            <div className="space-y-4 animate-pulse">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-24 bg-white rounded-xl border border-gray-100"
                ></div>
              ))}
            </div>
          ) : (
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1">
                    <h3
                      className={`font-bold text-lg font-serif transition-colors ${
                        expandedIndex === index
                          ? "text-pakGreen-800"
                          : "text-gray-900"
                      }`}
                    ...
                    <div className="px-5 pb-5 pt-0 ml-[3.5rem]">
                      ...
                      <ul className="space-y-2 mb-4">
                        {item.outline.map((point, i) => (
                          ...
                          >
                            <span className="text-pakGreen-500 font-bold">
                              •
                            </span>
                            <span>{point}</span>
                          </li>
          ...
          <div className="bg-white w-full h-full md:max-w-6xl md:h-[90vh] md:rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="px-4 md:px-6 py-4 border-b border-gray-100 flex items-start justify-between bg-white z-20 shadow-sm sticky top-0">
              <div className="flex-1 min-w-0 mr-4">
                <h2 className="text-lg md:text-xl font-bold font-serif text-gray-900 leading-tight break-words">
                  {activeEssay.title}
                </h2>
                <p className="text-xs text-gray-500 mt-1">
                  {activeEssay.content.split(/\s+/).length} words •{" "}
                  {Math.ceil(activeEssay.content.split(/\s+/).length / 200)} min
                  read
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <div className="hidden md:flex items-center bg-gray-100 rounded-lg p-1">
                  <button
                    onClick={() => {
                      console.log("Setting text size to SM");
                      setTextSize("sm");
                    }}
                  ...
                  <button
                    onClick={() => {
                      console.log("Setting text size to BASE");
                      setTextSize("base");
                    }}
                    className={`p-1.5 rounded-md ${
                      textSize === "base"
                  ...
                  <button
                    onClick={() => {
                      console.log("Setting text size to LG");
                      setTextSize("lg");
                    }}
                    className={`p-1.5 rounded-md ${
                      textSize === "lg"
                  ...
                  <button
                    onClick={() => {
                      console.log("Setting text size to XL");
                      setTextSize("xl");
                    }}
                    className={`p-1.5 rounded-md ${
                      textSize === "xl"
                ...
                <button
                  ...
                  className={`p-2 rounded-full transition-colors ${
                    showOutlineInModal
                      ? "bg-pakGreen-50 text-pakGreen-600"
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <div className="w-full md:w-80 bg-gray-50 border-r border-gray-100 overflow-y-auto flex-shrink-0 absolute md:static inset-0 z-10 md:z-auto md:block">
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <button
                        onClick={() => setShowOutlineInModal(false)}
                    ...
                    <nav className="space-y-1">
                      {activeEssay.outline.map((point, i) => (
                        <div
                          key={i}
                          className="flex gap-3 text-sm text-gray-600 py-2 border-b border-gray-100 last:border-0"
                        >
                          <span className="text-pakGreen-500 font-bold font-serif text-lg leading-none mt-0.5">
                            •
                          </span>
              ...
              <div
                ...
                onClick={() =>
                  window.innerWidth < 768 && setShowOutlineInModal(false)
                }
              >
                <div className="max-w-3xl mx-auto p-6 md:p-12">
                  {/* MANUAL text sizing - This WILL work */}
                  <div
                    ...
                    <ReactMarkdown
                      components={{
                        h1: ({ node, ...props }) => (
                          <h1
                            className={`
              font-bold mb-6 pb-2 border-b-2 border-pakGreen-100 text-pakGreen-900
              ${
                textSize === "sm"
                  ? "text-xl"
                  : textSize === "base"
            ...
            `}
                        ...
                        h2: ({ node, ...props }) => (
                          <div className="mt-8 mb-4">
                            <h2
                              className={`
                font-bold text-pakGreen-800 relative inline-block pl-4
                ${
                  textSize === "sm"
                    ? "text-lg"
                    : textSize === "base"
            ...
            `}
                        ...
                        p: ({ node, ...props }) => (
                          <p
                            className="mb-4 text-gray-800 text-justify indent-8"
                            {...props}
                          />
                        ),
                        ul: ({ node, ...props }) => (
                          <ul
                            className="space-y-2 mb-4 ml-2 list-none"
                            {...props}
                          />
                        ),
                        li: ({ node, ...props }) => (
                        ...
                        blockquote: ({ node, ...props }) => (
                          <blockquote
                            className="border-l-4 border-pakGreen-500 pl-6 italic text-gray-700 my-6 bg-gray-50 py-4 pr-6 rounded-r-xl"
                            {...props}
                          />
                        ),
                      }}
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end">
              <button
                onClick={() =>
                  onSaveNote(activeEssay.title, activeEssay.content)
                }
